...
int modc__size = 12;//modc__size*2+1 = minimum block width //Oda modified
if(aligned_blocks->pos_extnt[i].right - aligned_blocks->pos_extnt[i].left < modc__size*2){
	aligned_blocks->pos_extnt[i].right = MIN(msa->dimensions->query_length-1,MAX(aligned_blocks->pos_extnt[i].right,i+modc__size));
	aligned_blocks->pos_extnt[i].left =  MAX(0,MIN(aligned_blocks->pos_extnt[i].left,i-modc__size));
}
        }
    }
}
